...
resource "aws_security_group" "cluster" {
  ...
  ingress {
    description = "Allow all traffic between cluster control plane and nodes"
    from_port   = 0
    to_port     = 0
    protocol    = "-1"
    self        = true
...
# Grant the Terraform caller (and any other supplied ARNs) cluster-admin access
# so the kubernetes/helm providers can authenticate during apply.
